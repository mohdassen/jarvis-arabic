...
async function poll(){try{const r=await fetch('/setup/api/oauth/status',{credentials:'same-origin',cache:'no-store'});const j=await r.json();out.textContent=j.output||j.state;if(j.state==='running')setTimeout(poll,1000);else if(j.state==='success')out.textContent+='\\n\\nSUCCESS - ChatGPT connected. Open /openclaw';}catch(e){out.textContent=String(e)}}
poll();
</script></body></html>`);
});
...
app.post("/setup/api/oauth/start", requireSetupAuth, async (_req, res) => {
  ...
  // Railway is headless, so run the CLI inside util-linux `script`, which provides
  // a real PTY while still allowing us to stream the device URL/code to the web UI.
  const cmd = [OPENCLAW_NODE, ...args].map(v => JSON.stringify(String(v))).join(" ");
  const proc = childProcess.spawn("/usr/bin/script", ["-qefc", cmd, "/dev/null"], { env: {...process.env, OPENCLAW_STATE_DIR: STATE_DIR, OPENCLAW_WORKSPACE_DIR: WORKSPACE_DIR}, stdio:["ignore","pipe","pipe"] });
...
app.get("/setup/api/oauth/status", requireSetupAuth, (_req,res) => { res.set("Cache-Control","no-store"); res.json({ok:true,state:jarvisOauth.state,exitCode:jarvisOauth.exitCode,output:jarvisOauth.output}); });
